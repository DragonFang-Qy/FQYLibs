...
import com.fqy.fqylibs.interfaces.OnUpDownPullRefreshListener;

/**
 * @Title: DownRefershScrollView.java
 * @Package com.fqy.fqylibs.view
...
public class UpDownRefershListView extends ListView implements
	...
	private OnUpDownPullRefreshListener upDownPullRefreshListener;
	...
	private ImageView header_arrow_iv; // 头布局的剪头
	private ProgressBar header_pb; // 头布局的进度条
	private TextView header_state_tv; // 头布局的状态
	private TextView header_time_tv; // 头布局的最后更新时间
	 ...
	 * 
	 * @author: Fang Qingyou
	 * @date 2015年6月6日上午9:50:46
	 */
	private void initHeadView() {
		headerView = View.inflate(getContext(), R.layout.listview_header, null);

		header_arrow_iv = (ImageView) headerView
				.findViewById(R.id.header_arrow_iv);
		header_pb = (ProgressBar) headerView.findViewById(R.id.header_pb);
		header_state_tv = (TextView) headerView
				.findViewById(R.id.header_state_tv);
		header_time_tv = (TextView) headerView
				.findViewById(R.id.header_time_tv);

		// 设置最后刷新时间
		header_time_tv.setText(String.format(
				getResources().getString(R.string.header_time),
				getLastUpdateTime()));
		...
		if (scrollState == SCROLL_STATE_IDLE
			...
			if (isScrollToBottom && !isLoadingMore) {
				...
				if (upDownPullRefreshListener != null) {
					upDownPullRefreshListener.onUpPullRefreshListener();
				}
			}
		}
	}
	...
	public void onScroll(AbsListView view, int firstVisibleItem,
		...
		if (getLastVisiblePosition() == (totalItemCount - 1)) {// totalItemCount-1
																// 总条目数减头布局
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (currentState == RELEASE_REFRESH) {
				...
				if (upDownPullRefreshListener != null) {
					upDownPullRefreshListener.onDownPullRefreshListener();
					; // 调用使用者的监听方法
				}
	...
	private void refreshHeaderView() {
		...
		case DOWN_PULL_REFRESH: // 下拉刷新状态
			header_state_tv.setText("下拉刷新");
			header_arrow_iv.startAnimation(downAnimation); // 执行向下旋转
			break;
		case RELEASE_REFRESH: // 松开刷新状态
			header_state_tv.setText("松开刷新");
			header_arrow_iv.startAnimation(upAnimation); // 执行向上旋转
			break;
		case REFRESHING: // 正在刷新中状态
			header_arrow_iv.clearAnimation();
			header_arrow_iv.setVisibility(View.GONE);
